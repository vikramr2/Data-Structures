/*From the book: Data Structures & Algorithms, Michael Goodrich 
with the complete exercises for this class*/
...
void testCard() { // CreditCard test function
	...
	CreditCard::Date due = {4, 1, 2020};
	CreditCard::Date pay = {4, 2, 2020};
	
	for (int j=1; j <= 18; j++) { // make some charges
	...
	for (int i=0; i < 3; i++) { // make more charges
		...
		while (wallet[i]->getBalance() > 100.0) {
			wallet[i]->makePayment(100.0, due, pay);
